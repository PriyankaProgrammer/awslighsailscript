...
echo
echo "Welcome To GitLab GitOps demo on AWS Lightsail ..."
...
select opt in "${options[@]}" "Quit"; do 
  ...
  5) echo "You picked $opt "; export AWS_REGION=us-east-1;     export AWSLS_VM_SIZE_SUFFIX=_2_0; break;;
...
export AWS_SIZE_NANO="nano${AWS_VM_SIZE_SUFFIX}"
export AWS_SIZE_MICRO="micro_2_0"
...
create-vm $VM_PREFIX-ubuntu $AWS_SIZE_MICRO
...
open-vm-standard-network-port $VM_PREFIX-ubuntu
open-vm-specific-network-port $VM_PREFIX-ubuntu 8080 8080
...
for vm in ubuntu; do
  ...
  echo "ssh -o StrictHostKeyChecking=no -i mylab.key ubuntu@$VM_IP" > ssh-mylab-$vm.sh
...
for vm in ubuntu; do
  ...
  echo "  User ubuntu" >> mylab-ssh-config
...
#  until ssh $SSH_OPTS -i mylab.key ubuntu@$VM_IP true; do
#      sleep 5
#  done
#  scp $SSH_OPTS -i mylab.key mylab.key ubuntu@$VM_IP:~/.ssh/
#  scp $SSH_OPTS -i mylab.key mylab-ssh-config ubuntu@$VM_IP:~/.ssh/config
#  scp $SSH_OPTS -i mylab.key setup/$vm/*.*  ubuntu@$VM_IP:~/
#done 
#
...
#ssh $SSH_OPTS -i mylab.key ubuntu@$HARBOR_IP mkdir -p devsecops/{jenkins,sonarqube,anchore,longhorn}
#scp $SSH_OPTS -i mylab.key mylab_vm_list.txt ubuntu@$HARBOR_IP:~
#scp $SSH_OPTS -i mylab.key setup/jenkins/*.* ubuntu@$HARBOR_IP:~/devsecops/jenkins
#scp $SSH_OPTS -i mylab.key setup/sonarqube/*.* ubuntu@$HARBOR_IP:~/devsecops/sonarqube
#scp $SSH_OPTS -i mylab.key setup/anchore/*.* ubuntu@$HARBOR_IP:~/devsecops/anchore
#scp $SSH_OPTS -i mylab.key setup/longhorn/*.* ubuntu@$HARBOR_IP:~/devsecops/longhorn
#
#
## install rancher now?
#function install_rancher() {
#  RANCHER_IP=`cat mylab_vm_list.txt | grep $VM_PREFIX-rancher | cut -d '|' -f 4 | xargs`
#  ssh -o StrictHostKeyChecking=no -i mylab.key ubuntu@$RANCHER_IP sh 99-one-step-install-rancher.sh
#}
